...
<br><br>
해당 코드를 보며 생각해봤을 때,<br>`viewWillAppear(_ animated: Bool)` 에서 `ViewController` 를 `Present` 하게되면 뷰가 다 보여지지 않은 상태에서 `Modal`에서 보여지는`ViewController`가<br>계층구조에 올라가는 것이 아닐까 라는 생각이 들며
<br><br>
